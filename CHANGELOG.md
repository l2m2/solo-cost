...
- 任务详情页：点击项目任务列表里的任务标题、工时图标，或全局搜索命中任务，都会跳转到该任务自己的页面（原来是弹对话框），描述、备注、时间线收在同一处查看和编辑；Dashboard「待办」卡片里的任务标题现在也跳这里，不再只是跳回项目页
- 任务详情页分左右两栏：左侧是任务描述与动态，右侧是负责人、模块、工时、日期等信息。描述和信息平时都是直接可读的文字，点各自的「编辑」才切换成表单，不必为了看一眼负责人先面对一堆输入框
- 任务详情页右上角突出显示实际工时，下方标注预估工时与超出／剩余多少，超时会标红
- 任务时间线：手写备注、状态流转（开始/暂停/恢复/完成/关闭）与工时记录按时间合并成一条叙事，最新的在最上面；备注和工时条目可以在时间线上直接编辑或删除，状态变化是不可更改的记录
- 升级后老任务和禅道导入的任务，时间线不是空的——迁移时会按创建日期、实际开始/完成日期把历史事件补齐
- 手动删除时间线上的一条备注需要二次确认，且删除后不进回收站、无法恢复；删除任务本身仍是软删除，从回收站恢复任务时其备注与时间线会一并恢复

### Changed
- 编辑任务与工时清单两个对话框下线，相关操作并入任务详情页；列表上仍保留开始/暂停/恢复/完成/关闭这几个快捷按钮，无需进详情页也能随手处理
- Dashboard 待办：暂停中的任务不再被判定为逾期（它是卡在外部依赖上，不是拖延），排序上排在活跃任务之后、已完成任务之前
- 新建任务对话框去掉「状态」一栏：它一直是不可填写的只读框，新建的任务必然是「待办」
...
### Fixed
- 各类录入对话框（新建/编辑任务、成本、收款节点、工时、项目、客户、成员、分类、公司、模块，以及开始/完成任务和禅道导入）在已填写内容后，误点对话框外侧或误按 Esc 不会再直接关闭、导致重新录入；未填写任何内容时行为不变，仍可随手关掉。点「取消」或右上角关闭按钮照旧直接关闭，不额外确认

## 0.4.0 (2026-07-28)

### Added
- 仪表盘将「待办」独立为单独的 Tab 页：从「总览」中拆出，跨项目汇总全部未关闭任务（不再限制 10 条），支持按状态、负责人筛选与分页，新增预估/实际工时列，已完成的任务可在此一键关闭（归档）
- 任务列表中已完成的任务新增「关闭」按钮，可一键关闭（归档），无需再打开任务详情

### Changed
- 新建任务时「预估工时」改为必填
- 完成任务时必须已指定负责人，且任务总工时（已有 + 本次）须大于 0，避免完成的任务没有工时记录

### Fixed
- 工时表格「编辑工时」和「删除」按钮此前会挤成两行，现调整为同一行显示
- 删除任务、成本、收款节点、工时、模块、客户、成员，以及彻底删除回收站条目、恢复备份、导出明文库等操作，此前的确认提示不会真正拦截、点了就直接执行，现在均会正常弹出确认框
...
### Changed
- 仪表盘与应用外壳换上「账本」视觉：宣纸底、墨黑/朱砂红、中文衬线数字，仪表盘核心「已收到手」旁盖朱红印章，侧边栏「沃工本」改衬线标识、选中项朱砂红；内容页保持简洁
...
- 任务表格默认隐藏「模块」列和「创建日期」列
- 应用默认最大化打开窗口
- 仪表盘应收表新增「客户」列，项目和节点名称列适当加宽
- 项目收款表操作列加宽，避免「标为已收」按钮显示不全

### Fixed
- 修复编辑任务时实际开始/完成时间字段不显示已有值的问题（date 输入改 datetime-local）
- 修复 release build 的类型错误
...
- 年到手明细改用含税金额展示，合并为单表（项目·节点名·实收含税·实收日期·提成·到手）
